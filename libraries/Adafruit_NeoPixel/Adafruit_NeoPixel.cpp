...
//----
  ...
  if (is800KHz) {
    ...
    SysTick->VAL = 0;        // Set to start value
    for (;;) {
      GPIO_BOP(gpioPort) = (uint32_t)gpioPin;
      cyc = (pix & mask) ? t1 : t0;
      while (SysTick->VAL > cyc)
        ;
      GPIO_BC(gpioPort) = (uint32_t)gpioPin;
      if (!(mask >>= 1)) {
        if (p >= end)
          break;
        pix = *p++;
...
#elif defined(ARDUINO_ARCH_GD32)
    const PinName pinname = DIGITAL_TO_PINNAME(p);
    gpioPort = gpio_port[GD_PORT_GET(pinname)];
    gpioPin =  gpio_pin[GD_PIN_GET(pinname)];
#endif
}
